...
def make_azure_blueprint(
    ...
    *,
    ...
    authorization_url_params["response_mode"] = "query"
    token_url_params = {}
    token_url_params["scope"] = "openid profile offline_access " + \
        ''.join(_scope)
    token_url_params["client_id"] = client_id
    ...
    azure_bp = OAuth2ConsumerBlueprint(
        ...
        # for initial request scope = openid, for authorization code actual scope is used through token url params
        scope=["openid"],
    ...
    class UserSessionStorage(BaseStorage):
        ...
        def get(self, blueprint):
            with self.__lock:
                ...
                if not user_id:
                    return None
                key = self.key.format(userid=user_id)
    ...
    azure_bp.storage = UserSessionStorage()
